...
class ComfyUIModule(loader.Module):
    ...
    def _extract_chat_id(self, obj) -> Optional[int]:
        # Совместимость с InlineCall разных реализаций (у некоторых нет .chat_id)
        ...
        if msg is not None:
            val = getattr(msg, "chat_id", None)
            if isinstance(val, int) and val != 0:
                return int(val)
            with contextlib.suppress(Exception):
                return int(utils.get_chat_id(msg))
        return None
    ...
    async def _open_models_menu(self, target):
        ...
        buttons.append(
            [
                {"text": "📥 Скачать модели", "callback": self._open_download_root, "args": (), "style": "primary"},
    ...
    async def _bg_dl(self, msg, url, filename):
        ...
        try:
            async with aiohttp.ClientSession() as s:
                async with s.get(url, timeout=None, headers={"User-Agent": "Mozilla/5.0"}) as r:
